...
window.CodeMirror = (function() {
  ...
  CodeMirror.prototype = {
    ...
    getTokenAt: function(pos) {
      ...
      while (stream.pos < pos.ch && !stream.eol()) {
        ...
        var style = mode.token(stream, state, null, this);
      }
  ...
  // classes.
  function runMode(cm, text, mode, state, f, lineNum) {
    ...
    while (!stream.eol()) {
      if (stream.pos > cm.options.maxHighlightLength) {
        ...
        style = mode.token(stream, state, lineNum, cm);
      }
  ...
  function highlightLine(cm, line, state, lineNum) {
    // A styles array always starts with a number identifying the
    // mode/overlays that it is based on (for easy invalidation).
    var st = [cm.state.modeGen];
    // Compute the base array of styles
    runMode(cm, line.text, cm.doc.mode, state, function(txt, style) {st.push(txt, style);}, lineNum);
  ...
  function getLineStyles(cm, line) {
    if (!line.styles || line.styles[0] != cm.state.modeGen) {
      var number = lineNo(line);
      line.styles = highlightLine(cm, line, line.stateAfter = getStateBefore(cm, number), number);
    }
    return line.styles;
  }
  ...
  function processLine(cm, line, state) {
    ...
    while (!stream.eol() && stream.pos <= cm.options.maxHighlightLength) {
      mode.token(stream, state, null, cm);
      stream.start = stream.pos;
    }
  }
